...
class JSONTagExtractor:
    ...
    def __init__(self, model_name: str = "/mnt/data1/bge-m3", target_file_path: str = "/home/linzijian/pyproject/create_data/target_database/target.xlsx"):
        """
        初始化提取器
    ...
    def extract_tags(self, query: str) -> Dict[str, Union[str, None]]:
        """从查询中提取JSON标签"""
        # 定义歧义词列表和特殊目标词列表
        ...
        for i, phrase_emb in enumerate(phrase_embeddings):
            op, score, original_phrase = self._find_best_match(
                ...
                self.operation_list,
                phrases[i],  # 传入原始短语
                threshold=0.88  # 提高非歧义词的匹配阈值
            )
            
            if op and score > 0:
                if op in ambiguous_words:
                    ambiguous_candidates.append((op, score, original_phrase))
                else:
                    op_candidates.append((op, score, original_phrase))
        ...
        if not op_candidates:
            return {"operation": None, "target": None, "original_operation": None, "original_target": None}
