...
class JSONTagExtractor:
    ...
    def extract_tags(self, query: str,  ope_threshold:float=0.88, tar_threshold:float=0.85) -> Dict[str, Union[str, None]]:
        """从查询中提取JSON标签"""
        # 定义歧义词列表和特殊目标词列表
        ...
        # print(op_candidates)
        # 选择最佳操作词
        # best_operation, ope_score , original_operation, _ = sorted(op_candidates, key=lambda x: (x[1], x[3]), reverse=True)[0]
        tar_score_ope = sorted(op_candidates, key=lambda x: (x[1], x[3]), reverse=True)
        print(tar_score_ope)
        # len_ = len(tar_score_ope)
        # best_index = 0
        # for i in range(len_):
        #     if i < len_ - 1 and tar_score_ope[i][0] == tar_score_ope[i+1][0]:
        #         if tar_score_ope[i][3] < tar_score_ope[i+1][3]:
        #             best_index = i + 1
        # best_operation, ope_score , original_operation, _ = tar_score_ope[best_index]
        best_operation, ope_score , original_operation, _ = tar_score_ope[0]
        ...
        if target_phrases:
            ...
            for i, phrase_similarities in enumerate(similarity_matrix):
                ...
                # 应用四舍五入保持精度
                score = round(score, 5)
                # print(unique_phrases[i], "-------", self.targets[best_idx], "-------",score)
                # 应用阈值过滤
